define("moe/onImagesLoad/1.2.2/onImagesLoad-debug", [], function(require, exports, module) {
    /**
    ...
    (function($) {
        $.fn.onImagesLoad = function(options) {
                    ...
                    $imgs.each(function(i) {
                        ...
                        $(this).bind("load", function() {
                            if (jQuery.inArray(i, loadedImages) < 0) {
                                ...
                                loadedImages.push(i);
                                //keep a record of images we've seen
                        ...
                        if (!_ie) {
                            ...
                            this.src = orgSrc;
                            // we have to trigger, or the callback will not fire
                            $(this).trigger("load");
                        }
                    });
                }
            ...
            var imgAry = [];
            //only used if self.opts.all exists
            self.each(function() {
                if (self.opts.each) {
                    var $imgs;
                    if (this.tagName == "IMG") {
            ...
            if (self.opts.all) {
                self.bindEvents(imgAry, this, self.opts.all);
            }
        ...
        $.fn.onImagesLoad.defaults = {
            all: null,
            //function - the function to invoke when all images that $(yourSelector) encapsultaes have loaded (invoked only once per selector. see documentation)
            each: null,
            //function - the function to invoke when each item that $(yourSelector) encapsultaes has loaded (invoked one or more times depending on selector. see documentation)
